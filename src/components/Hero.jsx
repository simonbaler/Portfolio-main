...
import heroVideo from '../assets/hero video/simon-hero.mp4.mp4';
...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    // Autoplay the welcome video (muted for browser autoplay policies)
    if (videoRef.current) {
      videoRef.current.play().catch(() => {
        // Browser blocked autoplay; user can click "Play Reel" to start
        setIsPlaying(false);
      });
    }
  }, []);
      ...
      <div className="absolute inset-0 z-20 px-6 pb-20 md:pb-[8%] md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end text-left w-full">
        ...
        <div className="flex flex-col items-start text-left max-w-2xl w-full">
          ...
          >
            ...
            {/* GitHub Button (replaces Resume Download) */}
            <a 
...
{/* Right Side: Play Video Button */}
        ...
        >
          ...
          >
            <div className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-white/30 bg-black/20 backdrop-blur-md flex justify-center items-center group-hover:scale-110 group-hover:bg-white group-hover:text-black transition-all duration-300">
              {isMuted ? (
                // Muted icon
            ...
            <span className="text-white text-[8px] md:text-[10px] font-bold tracking-widest uppercase opacity-70">
              {isMuted ? "Muted" : "Sound"}
            </span>
          </div>
        </div>
